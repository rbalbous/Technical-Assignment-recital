...
def createHoles(maze, w, h, prevCell, height, width, display = False):
    ''' 
    ...
    if (num == 0):
        maze[w][h].north = 0
        createHoles(maze, w, h - 1, 3, height, width, display)
    if (num == 1):
        maze[w][h].east = 0
        createHoles(maze, w + 1, h, 4, height, width, display)
    if (num == 2):
        maze[w][h].south = 0
        createHoles(maze, w, h + 1, 1, height, width, display)
    if (num == 3):
        maze[w][h].west = 0
        createHoles(maze, w - 1, h, 2, height, width, display)
    # Runs the same cell until all nieghbors are visited in different directions
    return createHoles(maze, w, h, 0, height, width, display)

def createMaze(height, width, display = mazeDisplay):
    maze = initMaze(height, width)
    createHoles(maze, 0, 0, 0, height, width , display = wallsDisplay)
    if (mazeDisplay == True):
        printMaze(maze, None, height, width)
    return maze

if __name__ == "__main__":
    createMaze(30, 40, display = True)
